    ...
    for sl_idx in range(len(shorelines)):
        ...
        for transect_idx, key in enumerate(transects.keys()):
            ...
            if settings.get('CASS', False):
                ...
                if key in settings.get('transects_to_plot', []) and dotprods.size != 0: # and info == CASS_V2.AMBIGUOUS_SHORELINE:
                    CASS_V2.plot_intersections(key, sl, intersecting_sl, normals, dotprods, transects[key],
                                               str(output['dates'][sl_idx])[:10], settings, im_datum, im_rgb, intersections)
